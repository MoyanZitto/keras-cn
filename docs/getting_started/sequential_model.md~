...
* 传递一个<code>batch_input_shape</code>的关键字参数给第一层，该参数包含数据的batch大小。该参数在指定固定大小batch时比较有用，例如在[<font color=#FF0000>stateful RNNs</font>]()中。事实上，Keras在内部会通过添加一个None将input_shape转化为batch_input_shape
...
现在你已经学会定义几乎任何Keras的模型了，对于不能通过Sequential和Merge组合生成的复杂模型，可以参考[<font color=#FF0000>泛型模型API</font>]()

## 编译

在训练模型之前，我们需要通过<code>compile</code>来对学习过程进行配置。<code>compile</code>接收三个参数：

* 优化器optimizer：该参数可指定为已预定义的优化器名，如<code>rmsprop</code>、<code>adagrad</code>，或一个<code>Optimizer</code>类的对象，详情见[<font color=#FF0000>optimizers</font>]()

* 损失函数loss：该参数为模型试图最小化的目标函数，它可为预定义的损失函数名，如<code>categorical_crossentropy</code>、<code>mse</code>，也可以为一个损失函数。详情见[<font color=#FF0000>objectives</font>]()
...
Keras以Numpy数组作为输入数据和标签的数据类型。训练模型一般使用<code>fit</code>函数，该函数的详情见[<font color=#FF0000>这里</font>]()。下面是一些例子。
...
<pre><code># for a multi-input model with 10 classes:
...
请FAQ中关于[<font color=#FF0000>状态LSTM</font>]()的部分获取更多信息
