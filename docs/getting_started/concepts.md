...
shape：形状，一个n阶张量的shape是一个长为n的tuple，依次描述了各个维度的大小。

batch：批，一组样本构成的集合，进行梯度下降的单位

epoch：轮，遍历一次样本集训练一次称为一轮，网络往往要训练很多轮才能有较好的效果

element-wise：逐元素，这种类型的操作是逐个元素进行的，如将矩阵的每一个值都加1就是一个element-wise的操作，但两个矩阵做矩阵乘法就不是element-wise

nb_xxx: 这个不是正文内容而是Keras命名习惯了，以nb开头的参数代表“number of xxx”，如nb_epoch代表训练的轮数

metrics：指标，指的是评价网络性能的一些度量，例如loss，准确率等

loss：损失函数或目标函数，也指目标函数的函数值，这个值表达了当前预测结果与真值之间的偏差，因此我们将其译为误差。

stateful RNN：状态RNN，即每个batch的样本最后状态用作下一个batch相应样本最初状态的RNN。似乎翻译为状态保持RNN比较合适，本文档暂时翻译做状态RNN
