...
在上一版本的Keras中，你可以通过```layer.get_ouput()```方法来获得层的输出张量，或者通过```layer.output_shape```获得其输出张量的形状。这个版本的Keras你仍然可以这么做（除了```layer.get_ouput()```被```ouput()```替换）。但如果一个层与多个输入相连，会出现什么情况呢？
...
对于```input_shape```和```output_shape```也是一样，只要一个层只有一个节点，或只要所有的节点都有相同的输入或输出形状，那么```input_shape```和```output_shape```都是定义良好的，并只反回一个值。但是，例如你把一个相同的```Convolution2D```应用于一个大小为\(3,32,32\)的数据，然后又将其应用于一个\(3,64,64\)的数据，那么你就需要显式的指定节点的下标了
