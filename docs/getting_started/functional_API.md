...
首先，我们将微博的数据转为（140，256）的矩阵，即每条微博有140个单词（微博有词数限制哦亲），每个单词的特征由一个256维的词向量（扩展的ASCII码编码了最常见的）表示，向量的每个元素为1表示某个字符出现，为0表示不出现。

【Tips】之所以是（140，256）是因为一条微博最多有140个单词（不信发条微博试试），而扩展的ASCII码表编码了常见的256个字符。原文中此处为Tweet，所以对外国人而言这是合理的。如果考虑中文字符，那一个单词的词向量就不止256了。
...
对于```input_shape```和```output_shape```也是一样，只要一个层只有一个节点，或只要所有的节点都有相同的输入或输出shape，那么```input_shape```和```output_shape```都是定义良好的，并只反回一个值。但是，例如你把一个相同的```Convolution2D```应用于一个大小为\(3,32,32\)的数据，然后又将其应用于一个\(3,64,64\)的数据，那么你就需要显式的指定节点的下标了
